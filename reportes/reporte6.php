...
{
    // Tabla coloreada
    function FancyTable($header, $fecha2)
    {
        ...
        $sql = "SELECT v.idventas, p.descripcion, p.numserie, v.precio, v.cantidad, v.total, concat(per.paterno,' ',per.materno) as trabajador FROM ventas as v, productosVendidos as p, Personal as per WHERE v.idproducto = p.idproducto AND v.IDPersonal = per.IDPersonal AND v.fecRegisto LIKE '%$fecha2%';";
...
$pdf->Cell(85,5,"Fecha: ".$fecha . " Hora ". $hora,1,1);
//AQUÍ LLAMAMOS LA FUNCION:
$pdf->SetFont('Arial','',8);
$pdf->FancyTable($header, $fecha2);
$pdf->Output();
?>